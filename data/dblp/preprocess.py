...
def read_node_docs(path):
    ...
    while 1:
        ...
        if line == '':
            break
        vec = line.strip().split(' ')
        node_docs_dict[vec[0]] = vec[1:]
    fin.close()
    return node_docs_dict

# --------------------------------------------------
# -------------------- test  -----------------------
# --------------------------------------------------
if __name__ == '__main__':
    G = nx.read_adjlist(path='dblp_adjlist.txt', create_using=nx.Graph())
    print('len(G.edges())', len(G.edges()))
    print('len(G.nodes())', len(G.nodes()))
    print('isolate', len(list(nx.isolates(G))))
    # G = nx.read_adjlist(path='cora_doub_adjlist.txt', create_using=nx.DiGraph())
     
    node_label_dict = read_node_label_downstream(path='dblp_label.txt')
    save_any_obj(obj=node_label_dict, path='dblp_node_label_dict.pkl')  # {node ID: degree, ...}
    ...
    save_nx_graph(nx_graph=Gs, path='dblp_dyn_graphs.pkl')

    # for retrieve explicit name/title only e.g. paper title
    node_docs_dict = read_node_docs(path='dblp_docs.txt')
    save_any_obj(obj=node_docs_dict, path='dblp_node_docs_dict.pkl')